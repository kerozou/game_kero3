	...
	if g.spinning {
		for i := 0; i < reelCount; i++ {
			for j := 0; j < 3; j++ { // 3つのスプライトを描画してスライド効果を実現
				...
				screen.DrawImage(reelImage2.SubImage(image.Rect(symbolWidth*i, symbolIndex*symbolHeight, symbolWidth*i, (symbolIndex+1)*symbolHeight)).(*ebiten.Image), op)
			}
		}
	}
	...
	if !g.spinning {
		...
		for i := 0; i < reelCount; i++ {
			for j := 0; j < 3; j++ {
				...
				symbolIndex := g.reels[i][j]
				screen.DrawImage(reelImage2.SubImage(image.Rect(symbolWidth*i, symbolIndex*symbolHeight, symbolWidth*(i+1), (symbolIndex+1)*symbolHeight)).(*ebiten.Image), op)
			}
		}
...
func main() {
	...
	for i := 0; i < reelCount; i++ {
		for j := 0; j < 3; j++ {
			game.reels[i][j] = reelSymbols[i][j]
		}
	}
